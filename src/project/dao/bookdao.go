...
func GetPageBooksByPrice(pageNumber int, priceMin, priceMax float64) (page *model.Page, err error) {
	// get total records
	sqlQuery := "select count(*) from books where price between ? and ?"
	var totalRecords int
	row := utils.Db.QueryRow(sqlQuery, priceMin, priceMax)
	err = row.Scan(&totalRecords)
	// set up page size
	var pageSize int = 4
	// get pages
	...
	// get books in current page
	sqlGetBooks := "select id,title,author,price,sales,stock,img_path from books where price between ? and ? limit ?,?"
	rows, err := utils.Db.Query(sqlGetBooks, priceMin, priceMax, (pageNumber-1)*pageSize, pageSize)
	if err != nil {
		return nil, err
	}
	var books []*model.Book = make([]*model.Book, 0)
